# `test_copier_repo` developer documentation
...
# install test_copier_repo as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `test_copier_repo`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d test_copier_repo.XXXXXX)
git clone git@github.com:chStaiger/test_copier_repo .
...
Visit
[https://test.pypi.org/project/test_copier_repo](https://test.pypi.org/project/test_copier_repo)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d test_copier_repo-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple test_copier_repo
```
...
Don't forget to also make a [release on GitHub](https://github.com/chStaiger/test_copier_repo/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.